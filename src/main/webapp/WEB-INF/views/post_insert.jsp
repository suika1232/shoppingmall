...
<%@ include file="./parts/head.jsp" %>
	<table border=1>
		<%--  "<%=session.getAttribute("xx")%>" == "${sessionScope.xx}"  --%>
		<tr>
			<td>카테고리</td>
		...
		<tr>
			...
			<td><input type=text name=title id=title required> 
				...
				<c:if
					test="${sessionScope.member_id != null && sessionScope.member_class == 'Manager'}">
					<input type=hidden name=post_answer id=post_answer value=''> 
				</c:if> 
				<c:if
					test="${sessionScope.member_id != null && sessionScope.member_class == 'User'}">
					<input type=hidden name=post_answer id=post_answer value="답변대기"> 
				</c:if> 				
				<input type=hidden name=member_seq id=member_seq value="<%=session.getAttribute("member_seq")%>"> 
				<input type=hidden name=post_img id=post_img value="">
				<input type=hidden name=post_comment id=post_comment value=''></td>
		<tr>
			<td>내용</td>
			<td><textarea name="summernote" id=summernote required></textarea></td>
		</tr>
		<tr>
			<td colspan=2 align=center><input type=submit value=등록 id=btnPost>&nbsp&nbsp&nbsp  
...
.on('click','#btnPost',function(){
	$.ajax({url:'/post_upload',type:'post',
		dataType:'text',data:{prod_category_code:$('#prod_category_code').val(),title:$('#title').val(),summernote:$('#summernote').val(),
							  post_category:$('#post_category').val(),member_seq:$('#member_seq').val(),post_answer:$('#post_answer').val(),
							  post_img:$('#post_img').val(),post_comment:$('#post_comment').val()},
