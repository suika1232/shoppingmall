...
#orderForm{left:30%;top:5%;margin:0 auto}
...
<div id="section">	
<br><br>
<form action="/addOrder" method="post" id="fmt">
...
<tr> <th colspan=5><h1>주문서</h1></th> </tr>
	...
	<c:forEach var="cart" items="${carts}">
		<tr>
			<td>
				<h3>${cart.pid}</h3> <input name=prod_id type="hidden" value="${cart.pid}">
				<input type="hidden" name="cart_seq" value="${cart.cart_seq}">
 			</td>
			<td><h3>${cart.prod_name}</h3></td>
			<td><img src="${cart.prod_image}" style="width:80px;height:80px"></td>
			<td><h3>${cart.cart_qty}</h3> <input name=order_qty type="hidden" value="${cart.cart_qty}"> </td>
			<td><h3>${cart.cart_price}</h3> <input name=order_price type="hidden" value="${cart.cart_price}">  </td>
		</tr>
	</c:forEach>
		...
		<td colspan=5>	
			<table id="orderAddress">
				...
				<tr> 						  
					...
					<td>
						<input type="text" name=part3 id="sample6_detailAddress" placeholder="상세주소" value="${part3}">
